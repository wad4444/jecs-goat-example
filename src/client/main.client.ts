...
export interface ClientState {
	...
	readonly scheduler: Scheduler<[ClientState]>;
}
...
const client_systems = import_systems($getModuleTree("client/systems"));
const shared_systems = import_systems($getModuleTree("shared/systems"));

scheduler.addSystems(client_systems);
scheduler.addSystems(shared_systems);

(state as Writable<ClientState>).scheduler = scheduler;
